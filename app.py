...
if df is not None:
    ...
    if selected_cust:
        # FILTER DATA FOR THE SELECTED CUSTOMER ONLY
        cust_df = df[df[cust_col].astype(str) == selected_cust].copy()
        
        # --- PER-CUSTOMER PARETO ANALYSIS (80/20 Rule) ---
        # Calculate revenue per product for THIS customer
        ...
        # Filter products contributing to top ~80% revenue
        top_prods = sales[sales['Cum_Pct'] <= 0.81]['Material name'].unique()
        
        st.info(f"💡 Pareto Insight: Found {len(top_prods)} products driving ~80% of revenue for {selected_cust}.")
        
        # 3. Dynamic Product Selection based on Pareto
        selected_prod = st.selectbox(f"2. Select Top Product (80/20 List):", top_prods)
            ...
            if selected_cies:
                ...
                for cie in selected_cies:
                    ...
                    if len(actual) >= 2:
                        ...
                        fig.add_trace(go.Scatter(x=f_only['ds'], y=f_only['yhat'], mode='lines', line=dict(dash='dash'), name=f"Forecast - {cie}"))
                        ...
                        ai_insights.append(f"**CIE {cie}**: Trend is **{status}** ({growth:.1f}% vs last 12m).")
                ...
                if forecast_results:
                    st.subheader("2026 Detailed Forecast Table")
                ...
                st.warning("Please select at least one CIE code.")
