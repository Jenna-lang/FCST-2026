...
# --- CORE LOGIC FUNCTIONS (KEEPING YOUR ORIGINAL LOGIC) ---

def get_actual_avg_qty(df, year, quarter, prod, cie_col, cie_val):
    """Original Logic: Avg of months with data (>0) in a Quarter."""
...
def get_quarterly_growth_logic(cust_df, prod, cie_col, cie_val):
    """Original Logic: Growth based on Latest 2026 Q vs 2025 same Q."""
            ...
            with tab1:
                ...
                if len(p_plot) > 2:
                    ...
                    if not v_df.empty:
                        ...
                        # --- FIX: NO MORE NONE IN AVERAGE ROW ---
                        avg_row = pd.DataFrame({
                            'ds': ["AVERAGE"], # String label instead of pd.NA
                            'y': [v_df['y'].mean()], 
                            'yhat': [v_df['yhat'].mean()], 
                            'Variance %': [avg_v]
                        })
