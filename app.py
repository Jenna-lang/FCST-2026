...
def process_data(uploaded_file):
    try:
        ...
        if date_col in df.columns and qty_col in df.columns:
            ...
            st.error(f"Missing columns: {date_col} or {qty_col}")
            return None
    ...
    if df is not None:
        ...
        if selected_cust != "-- Select --":
            ...
            with tab1:
                ...
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=m_plot['ds'], y=m_plot['y'], name="Actual", line=dict(color='blue', width=3)))
                ...
                # --- TABLE & COMMENTARY (CRITICAL SECTION) ---
                st.subheader("🔢 Actual vs AI Forecast Variance")
                act_26 = m_plot[m_plot['ds'].dt.year == 2026]
                v_df = pd.merge(act_26, fcst_26[['ds', 'yhat']], on='ds', how='left')
                if not v_df.empty:
                    ...
                    # AI Commentary
                    last_var = v_df['Var %'].iloc[-1]
                    st.markdown("### 💡 AI Strategic Advice")
                    if last_var > 15:
                        st.warning(f"**Warning:** Actual demand is **{last_var:.1f}% higher** than AI forecast. Risk of IC shortage. Secure supply for Q3/Q4.")
                    elif last_var < -15:
                        st.error(f"**Action Required:** Demand is **{abs(last_var):.1f}% lower** than projected. Adjust inventory to prevent overstock.")
                    else:
                        st.success("Tình hình kinh doanh ổn định (Business is stable). Actuals are closely tracking the AI model.")
            ...
            with tab2:
                ...
                pivot_list = []
                max_act = m_plot['ds'].max() if not m_plot.empty else pd.Timestamp('2026-01-01')

                for p in top_prods:
                    p_trnd, p_yoy, p_rr = get_metrics(cust_df, p)
                    f_factor = (p_yoy if p_yoy != 0 else (p_trnd/100)) + (adj_growth / 100)
                    cies = cust_df[cust_df['Material name'] == p][cie_col].unique()
                    
                    for c in cies:
                        row = {'Product': p, 'CIE': c, 'Growth': f"{f_factor*100:.1f}%"}
                        for m_date in months_26:
                            ...
                            if act > 0:
                                ...
                                # Logic for New Projects (Historical average fallback)
                                row[m_str] = round(h25 * (1 + f_factor), 0) if h25 > 0 else round(p_rr * (1 + f_factor), 0)
                            else: row[m_str] = 0
